<script lang="ts">
  ...
  const skeletonProps = (path: string) => {
    ...
    <div class="ps-4 pe-4 pt-2 pb-2">
      <div class="flex flex-col gap-2 md:flex-row md:gap-4">
        {#if additionalPropertiesTitle}
          <div class="md:flex md:h-10 md:items-center md:pt-6">
            <span class="text-surface-950-50 text-sm">{additionalPropertiesTitle}</span>
          </div>
        {/if}
        <div class="min-w-0 flex-1">
          <div class="relative">
            ...
            <button
              type="button"
              class="btn btn-sm preset-filled absolute end-0 top-6 shrink-0 p-0"
    ...
    <div class="flex flex-col gap-2 ps-4 pe-4 pb-4">
      {#each additionalPropertyItems as element (element.propertyName)}
        {#if element.schema && element.uischema}
          <div class="relative" class:pt-2={element.propertyName === '' && control.enabled}>
            {#if control.enabled}
              <div class="absolute end-1 top-0 z-20 flex items-center">
                <button
                  type="button"
                ...
                <button
                  ...
                  class="hover:preset-tonal rounded-base text-surface-500 dark:text-surface-400 hover:text-error-500 dark:hover:text-error-400 inline-flex size-7 items-center justify-center disabled:cursor-not-allowed disabled:opacity-40"
                >
