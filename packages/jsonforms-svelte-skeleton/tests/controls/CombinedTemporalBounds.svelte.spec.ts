...
describe('intersection of temporal bounds', () => {
  for (const [format, entry, trigger] of [
    ['date', date, labels.date],
    ['time', time, labels.time],
    ['date-time', dateTime, labels.dateTime],
  ] as const) {
    it(`${format} preserves data when combined bounds leave no selectable values`, async () => {
      const value =
        format === 'date' ? '2026-06-15' : format === 'time' ? '12:00:00' : '2026-06-15T12:00:00';
      ...
      if (format === 'date') {
        await vi.waitFor(() => expect(unavailable(dayButton(15))).toBe(true));
      } else {
        await vi.waitFor(() =>
          expect(document.querySelectorAll('input.time-input').length).toBe(3),
        );
        expect(
          [...document.querySelectorAll<HTMLInputElement>('input.time-input')].every(
      ...
      it(`${format} calendar respects ${exclusive ? 'exclusive' : 'inclusive'} field bounds`, async () => {
        ...
        const { view } = mountControl({
          ...
          propertySchema: {
            ...
            format,
            ...(exclusive
              ? { formatMinimum: `2026-06-10${suffix}`, formatMaximum: `2026-06-20${suffix}` }
              : {
                  formatExclusiveMinimum: `2026-06-10${suffix}`,
                  formatExclusiveMaximum: `2026-06-20${suffix}`,
                }),
      ...
      it(`${format} picker prevents stepping beyond ${exclusive ? 'exclusive' : 'inclusive'} bounds`, async () => {
        ...
        const { view } = mountControl({
          ...
          propertySchema: {
            ...
            format,
            ...(exclusive
              ? { formatMinimum: `${prefix}12:00:10`, formatMaximum: `${prefix}12:00:50` }
              : {
                  formatExclusiveMinimum: `${prefix}12:00:10`,
                  formatExclusiveMaximum: `${prefix}12:00:50`,
                }),
            [exclusive ? 'formatExclusiveMinimum' : 'formatMinimum']:
