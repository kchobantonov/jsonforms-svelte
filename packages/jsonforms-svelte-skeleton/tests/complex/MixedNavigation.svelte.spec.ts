...
const openCustomer = async (value = { customer: 'Ada' }) => {
  ...
  getBySelector<HTMLElement>(container, '[data-part="item-trigger"]').click();
  await vi.waitFor(() =>
...
const chooseComboboxOption = async (container: HTMLElement, label: string) => {
  ...
  await vi.waitFor(() => {
    option = Array.from(document.querySelectorAll<HTMLElement>('[role="option"]')).find(
      // Skeleton keeps closed combobox content mounted. The root also offers
      // String, so only interact with the selected node's visible option.
      (candidate) =>
