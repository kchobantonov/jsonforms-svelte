...
const patternCache = new WeakMap<object, { signatures: string[]; patterns: RegExp[] }>();
...
export function additionalPropertySchema(
  ...
  if (!cached || !isEqual(cached.signatures, signatures)) {
    cached = { signatures, patterns: signatures.map((pattern) => new RegExp(pattern)) };
  ...
  const resolve = (schema: JsonSchema, seen = new Set<JsonSchema>()): JsonSchema => {
    if (!schema.$ref || seen.has(schema) || Object.keys(schema).length !== 1) return schema;
  ...
  if (!matches.length) {
    ...
    return resolve(
      normalize(
        fallback === undefined || fallback === true ? { additionalProperties: true } : fallback,
      ),
    );
  }
  ...
  for (const schema of schemas) {
    for (const [key, value] of Object.entries(schema)) {
      ...
      if (!(key in merged) || isEqual(merged[key], value)) {
        ...
        key === 'type' &&
        [merged[key], value].every((type) => type === 'integer' || type === 'number')
      ) {
        merged[key] = 'integer';
  ...
  if (
    ...
    !['string', 'number', 'integer', 'boolean', 'null'].includes(merged.type as string)
  ) {
    return conjunction;
  }
