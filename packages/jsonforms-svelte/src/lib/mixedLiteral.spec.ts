...
it('protects ordinary declared-property scopes while allowing dynamic literal editors', () => {
  const root = {
    definitions: { child: { type: 'object', properties: { 'a.b': { type: 'string' } } } },
  };
  expect(mixedHasUnsafeDeclaredScopes({ additionalProperties: true }, root)).toBe(false);
  expect(mixedHasUnsafeDeclaredScopes({ $ref: '#/definitions/child' }, root)).toBe(true);
  expect(
    mixedHasUnsafeDeclaredScopes({ additionalProperties: { $ref: '#/definitions/child' } }, root),
  ).toBe(true);
});
