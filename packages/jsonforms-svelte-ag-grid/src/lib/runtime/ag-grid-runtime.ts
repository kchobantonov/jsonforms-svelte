...
type CellHostExports = {
  refresh: (props: AgGridCellHostProps) => void;
};
...
export const createAppearanceTheme = (runtime: RuntimeModule): Theme => {
  const common = {
    // Quartz defaults this to "light", which prevents design-system
    // light-dark() tokens from following the surrounding mode.
  ...
  // Keep these as CSS variable references so design-system and light/dark
  // token changes apply without recreating the grid.
  return runtime.themeQuartz.withParams(common);
};
...
export const createGrid = async (
  ...
  return runtime.createGrid(
    element,
    {
      ...
      components: {
        ...(options.components ?? {}),
        JsonFormsDispatchCell: CellRenderer,
      },
    },
