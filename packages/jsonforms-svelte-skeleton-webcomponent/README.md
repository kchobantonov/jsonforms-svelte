...
A standalone Skeleton JSON Forms custom element built with Svelte 5. It includes
...
- Custom element: `jsonforms-svelte-skeleton`
- Browser ES module: `dist/jsonforms-svelte-skeleton.js`
...
<html lang="en">
  <head>
    ...
    <title>JSON Forms Skeleton Web Component</title>
    <style>
      body {
        max-width: 800px;
        margin: 2rem auto;
...
to a public directory such as `/vendor/jsonforms-svelte-skeleton/`. Preserve its layout;
copying only the entry JavaScript file is not sufficient. Then replace the CDN
import in the example with:

```js
import '/vendor/jsonforms-svelte-skeleton/jsonforms-svelte-skeleton.js';
```
...
`customElements.whenDefined('jsonforms-svelte-skeleton')` before assigning properties.
...
| `theme`             | `theme`            | Included color theme name; see below.                                                                               | `cerberus`         |
...
Included Skeleton themes: `catppuccin`, `cerberus`, `concord`, `crimson`, `fennec`,
`hamlindigo`, `legacy`, `mint`, `modern`, `mona`, `nosh`, `nouveau`, `pine`, `reign`,
`rocket`, `rose`, `sahara`, `seafoam`, `terminus`, `vintage`, `vox`, and `wintry`.
Set `form.theme = 'modern'` to switch theme independently of light/dark mode.
...
pnpm run wc:skeleton:build
pnpm run wc:skeleton:preview
```
