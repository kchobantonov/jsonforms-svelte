...
<div
  ...
  class={twMerge(
    ...
    isHorizontal
      ? 'bg-border hover:bg-primary focus:bg-primary w-px cursor-col-resize'
      : 'bg-border hover:bg-primary focus:bg-primary h-px cursor-row-resize',
...
>
  <div
    class={twMerge('absolute bg-transparent', isHorizontal ? 'h-full w-3' : 'h-3 w-full')}
    style={handleStyle}
  ></div>
</div>
