<script lang="ts">
  ...
  function handlePointerDown(event: PointerEvent, index: number) {
    ...
    const handlePointerMove = (nextEvent: PointerEvent) => {
      const currentPos = currentDirection === 'horizontal' ? nextEvent.clientX : nextEvent.clientY;
  ...
  $effect(() => {
    ...
    const updateSize = () => {
      ...
      containerSize = currentDirection === 'horizontal' ? node.offsetWidth : node.offsetHeight;
    };
  ...
  $effect(() => {
    ...
    if (containerSize < 1) {
      if (sizes.length !== registeredPanes) {
        sizes =
          getNormalizedInitialSizes() ??
          Array.from({ length: registeredPanes }, () => 100 / registeredPanes);
      }
      return;
    }
