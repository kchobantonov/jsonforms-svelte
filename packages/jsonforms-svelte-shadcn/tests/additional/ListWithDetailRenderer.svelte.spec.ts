...
describe('ListWithDetailRenderer', () => {
  ...
  it('renders empty state and enabled add button when no data is provided', () => {
    ...
    const addButton = Array.from(view.container.querySelectorAll<HTMLButtonElement>('button')).find(
      (button) => (button.getAttribute('aria-label') ?? '').toLowerCase().includes('add'),
    );
  ...
  it('keeps the list label empty when the child label value is removed', async () => {
    ...
    const label = item?.querySelector<HTMLElement>('[data-slot="item-title"]');
  ...
  it('constrains long item labels so the remove button remains in the row', () => {
    ...
    expect(label!.parentElement?.classList.contains('overflow-hidden')).toBe(true);
  });
