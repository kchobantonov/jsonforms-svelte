...
### 12.1 File-by-file responsibilities

#### `packages/editor/src/lib/editor/document/` — the model layer

- **`types.ts`** — the shared vocabulary every other file imports. Defines `EditorDocument` (§2: `schemaDraft`, `schema`, `uischema`, `data`, `meta`), `EditorMeta` (selection id, expanded-node sets for the Schema Tree and Canvas, active Inspector tab — all non-historied per §6.3), `SchemaDraftId` (`'draft-07' | '2019-09' | '2020-12'`), and `SchemaNodeInfo` (the resolved-type shape `scope-index.ts` produces per pointer). This file has no logic, only types/interfaces — it exists so `document/`, `commands/`, `dnd/`, and `components/` all agree on shapes without circular imports.
- **`schema-paths.ts`** — the only place that touches raw object traversal into `schema`. Exports `getAtPath(schema, path: string[])`, `setAtPath(...)`, `renameKeyAtPath(...)`, `deleteAtPath(...)`, `moveKeyAtPath(...)` (for reordering `properties` keys). Every one of these must satisfy §3.7 (merge-patch, not rebuild) — e.g. `setAtPath` sets exactly the key(s) it's told to and returns a new object via structural sharing (§6.1), never a filtered copy. This is also where `#`-rooted vs `#/$defs/Foo`-rooted paths (§3.6, §3.8) get normalized to the same internal path-array representation so the rest of the codebase doesn't special-case "is this the root or a definition."
- **`scope-index.ts`** — builds and memoizes `scopeToSchemaNode: Map<string, SchemaNodeInfo>` (§3.4) by walking the current `schema` once per change. Must call into `ref-resolution.ts` for any `$ref` node so the map's values are always the *dereferenced* type info, and must call into `dynamic-properties.ts` to decide whether an `object` node's `SchemaNodeInfo` should flag it as "dynamic-keyed" (relevant for §3.9's drag behavior and §8.1's `matchesSchema`). This is the single most depended-on file in `document/` — Inspector field availability, drag-and-drop type matching, and Monaco's data-pane schema registration (§9) all read from its output rather than re-deriving type info themselves.
- **`root-schema.ts`** — implements §3.6. Exports a `getRootNodeView(doc: EditorDocument): SchemaTreeNode` used by `SchemaTree.svelte` to render the synthetic `(root)` row, and `SetRootTypeCommand` (or a factory `createSetRootTypeCommand(newType)`) implementing the guided object↔array rewrite, including which existing properties get relocated into/out of `items.properties` and which resulting UI Schema scopes need the broken-scope flag. The confirmation-dialog copy itself lives in a component (§16 chrome string, not this file) — this file only produces the command and the "would this be destructive, and to what" analysis the dialog displays.
- **`draft-config.ts`** — the per-draft field-availability table from §3.5, e.g. `const AVAILABLE_KEYWORDS: Record<SchemaDraftId, Set<string>>`, plus references/paths to the bundled official meta-schema JSON files for each draft (used by both `ajv` setup in §11 and Monaco's `schema.json` completion in §9). Also holds the small piece of logic for "which `ajv` build/instance to construct for a given `schemaDraft`."
- **`ref-resolution.ts`** — implements §3.8's dereferencing. Exports `resolveRef(schema, pointer): { resolved: JsonSchemaFragment; cycle: boolean }` used by `scope-index.ts`, the Schema Tree (to render "→ Foo" reference rows and "↻ Foo (recursive)" cycle leaves), and the Inspector (to populate the read-only reference-node view). Also exports the `ExtractToDefinitionCommand` and `UnlinkRefCommand` factories referenced from `commands/schema-commands.ts` (implementation detail: whether these commands' *logic* lives here or in `schema-commands.ts` is a style choice — keeping the ref-graph-walking logic here and only the thin `EditorCommand` wrapper in `schema-commands.ts` avoids that file needing its own cycle-detection logic).
- **`dynamic-properties.ts`** — implements §3.9's tree/Inspector plumbing: `getDynamicPropertyPseudoChildren(objectNode)` (returns the "◆ Additional properties" / "◆ Pattern properties" rows for the Schema Tree), and helpers the Inspector's Dynamic Properties sub-section (§7.1) uses to read/write `additionalProperties` (boolean-or-schema) and `patternProperties` (regex-keyed map) without the Inspector component needing to know the raw JSON Schema shape of either keyword.
- **`merge-patch.ts`** — the shared implementation of §3.7's discipline: a small `patchAtPath(schema, path, patch: Partial<JsonSchemaFragment>)` (and a UI-Schema equivalent) that shallow-merges only the given keys into whatever's already at that path, leaving every other key (known or unknown/vendor) untouched. Every command in `schema-commands.ts`/`uischema-commands.ts` that edits an existing node's fields should route through this rather than writing ad hoc merge logic per command, so the §3.7 acceptance test only has to exercise one code path.
- **`commands/types.ts`** — the `EditorCommand` interface (§6.1) and `runTransaction(fn)` (§6.2). No concrete commands here — this is the contract every command file implements against.
- **`commands/schema-commands.ts`** — concrete commands for everything that mutates `schema`: `InsertPropertyCommand`, `RenamePropertyCommand` (must cascade into any UI Schema `scope`/`rule.condition.scope` referencing the renamed pointer — §7.1 — so this command likely composes a small UI-schema-scan-and-rewrite step internally, wrapped in `runTransaction` so it's one undo step), `DeletePropertyCommand`, `SetRootTypeCommand` (thin wrapper delegating to `root-schema.ts`), `ExtractToDefinitionCommand`/`UnlinkRefCommand` (thin wrappers delegating to `ref-resolution.ts`), and the `additionalProperties`/`patternProperties` mutation commands backing §7.1's Dynamic Properties sub-section.
- **`commands/uischema-commands.ts`** — concrete commands for everything that mutates `uischema`: `InsertElementCommand` (used by both §5.1 and §5.2's drop handlers), `MoveElementCommand` (§5.3 reordering/re-parenting), `UpdateElementPropsCommand` (Inspector General/Options tab edits, §7.2), `UpdateElementRuleCommand` (§7.3, both visual and raw-mode commits go through this one command type — the *mode* is Inspector UI state, not something the command needs to know about).
- **`commands/replace-commands.ts`** — `ReplaceSchemaCommand`/`ReplaceUiSchemaCommand`, the whole-document-slice replacements used exclusively by the Monaco → Editor sync direction (§9). Deliberately coarse-grained (no attempt at minimal diffing from raw text) since Monaco edits are relatively infrequent compared to drag/drop and don't need fine-grained undo steps beyond "what the document looked like before this paste/edit."
- **`history-store.svelte.ts`** — the runes-based `past`/`present`/`future` store (§6.1) and the `dispatch(command)` entry point every command above is actually invoked through. Also implements `runTransaction`'s batching (collecting synchronous `dispatch` calls within one microtask into a single history entry) and the `historied: false` opt-out for pure-UI `meta` mutations (§6.3).

#### `packages/editor/src/lib/editor/dnd/`

- **`payloads.ts`** — the `DragPayload` discriminated union (`{ kind: 'schema-property', pointer }`, `{ kind: 'palette-item', elementType, controlKind? }`, `{ kind: 'canvas-node', elementId }` — §5.1–5.3) and the `DropTarget` type describing where something landed (parent element id + index, or "inside" for containers).
- **`drop-handler.ts`** — the single `handleDrop(payload, target)` dispatcher (§5.4) every draggable source's drop event ultimately calls. Branches on `payload.kind`, and for `'palette-item'` control drops, owns opening the bind-or-create popover (§5.2) before deciding which commands to dispatch. This is intentionally the *only* place §5.1–5.3's branching logic lives.
- **`compatibility.ts`** — the static allowed-parent table (§5.3) — e.g. `Category` may only be moved under `Categorization` — consulted by `drop-handler.ts` before allowing a re-parent, and by `Canvas.svelte`/`CanvasNode.svelte` to decide which drop-zones to even render as valid during a drag (so illegal targets don't highlight).

#### `packages/editor/src/lib/editor/registrations/`

- **`control-definitions.ts`** — the default `EditorControlDefinition[]` (§8.1) covering standard JSONForms control kinds, plus the dynamic-object entry whose `matchesSchema` detects `additionalProperties`/`patternProperties` (§3.9, §8.2). Host apps override/extend this list via a prop/context on `EditorShell.svelte`; this file is only the shipped default.

#### `packages/editor/src/lib/editor/inspector/`

- **`schemas/`** — not code, but data: the internal JSON Schema + UI Schema pairs from §7.0, one pair per selection kind (`schema-property`, `ui-control`, `ui-layout`, `ui-categorization`, `rule`), and — per §3.5 — either one variant per `schemaDraft` or a single parametrized pair with drought-driven visibility rules, whichever approach was chosen. These are what actually get fed to the JSONForms renderer that *is* the Inspector (§7.0) — this directory is effectively "the Inspector's own form definitions," authored the same way a consumer of this whole editor would author any other form.
- **`renderers/`** — editor-specific custom JSONForms renderers registered only for the Inspector's own internal forms (§8.2 draws the generic-vs-specific line; these qualify as specific because they encode this editor's own data shapes): the enum-value chip editor (§7.1's repeatable enum/pattern-properties-list editing), and the composite rule-condition builder (§7.3's AND/OR/NOT recursive UI). Each is a normal JSONForms renderer/tester pair, just registered locally to the Inspector's own `JsonForms` instance rather than exported for host-app use.

#### `packages/editor/src/lib/editor/components/`

- **`SchemaTree.svelte`** — renders `root-schema.ts`'s root-node view plus the Definitions branch (§3.8), each node using `scope-index.ts`/`ref-resolution.ts`/`dynamic-properties.ts` for its icon/badge/pseudo-children. Owns the tree's own context-menu actions (add/rename/delete/duplicate/retype/reorder, §4) by dispatching the corresponding `schema-commands.ts` commands, and is a drag source registered with `dnd/payloads.ts`'s `'schema-property'` kind.
- **`Palette.svelte`** — static list driven by `registrations/control-definitions.ts`, grouped into Layouts/Controls/Custom (§4); a drag source for `'palette-item'` payloads.
- **`Canvas/Canvas.svelte`** — the drop target root for the canvas area; owns the top-level `handleDrop` wiring and renders the UI Schema tree by recursing into `CanvasNode.svelte` starting at `uischema`.
- **`Canvas/CanvasNode.svelte`** — one instance per UI Schema element (keyed by the element's editor-only id, §3.3), responsible for selection outline, drop-zone rendering between/inside its children (consulting `compatibility.ts`), and delegating its actual visual body to the matching component under `nodes/`.
- **`Canvas/nodes/`** — `ControlNode.svelte`, `LayoutNode.svelte` (Vertical/Horizontal), `GroupNode.svelte`, `CategorizationNode.svelte` — the per-element-type compact visual representations described in §4 ("label + selection outline + drop-zones," not a live-rendered field).
- **`Inspector/Inspector.svelte`** — hosts the actual JSONForms instance described in §7.0: given the current selection, picks the right pair from `inspector/schemas/`, binds the selected node's relevant data into it, renders it with the shadcn renderer set (plus the local custom renderers from `inspector/renderers/`), and maps its `onChange` output back into the appropriate command from `schema-commands.ts`/`uischema-commands.ts`.
- **`MonacoPane.svelte`** — one reusable component instantiated for all Monaco surfaces in the app (the three bottom-dock panes *and* every Inspector Advanced/raw-fragment/raw-rule spot, §9/§18.5) — takes a value, a JSON-schema-for-completion reference, and change/blur callbacks; owns the shared-instance/worker-reuse and disposal-on-unmount discipline called out in §9 so that concern lives in exactly one component rather than being reimplemented at each call site.
- **`LivePreview.svelte`** — wraps the shared shadcn-renderer package's `JsonForms` Svelte component (not this editor's own Canvas), fed directly and un-debounced from the `EditorDocument` store per §10.

#### `packages/editor/src/lib/editor/theme/` and `i18n/`

- **`theme/theme-context.ts`** — §15 in full: the `theme`/`resolvedTheme` Svelte context, the live `prefers-color-scheme` listener, and the Monaco-theme-sync side effect (calling `monaco.editor.setTheme` on change) so `MonacoPane.svelte` doesn't need its own theme-watching logic.
- **`i18n/locale-context.ts`** — §16.2/§16.3: the active `LocaleId` + `t(key, params?)` Svelte context, default-locale resolution (host prop → `navigator.language` → English fallback), and the missing-key-falls-back-to-English runtime behavior (the CI check that catches this at build time lives in tooling/scripts, not in this runtime file).
- **`i18n/locales/en.json`, `bg.json`** — the flat, namespaced chrome-string catalogs (§16.2) — data files, not logic; every other component under `components/` imports strings from these exclusively via `t()`, never inline literals.
- **`EditorShell.svelte`** — the top-level composition root: lays out the four zones + splitters (§4), renders the top bar (schema-draft selector §3.5, theme toggle §15, locale selector §16.3), and is where a host app's overrides (custom `EditorControlDefinition[]`, initial `EditorDocument`, `theme`/`locale` props) actually get threaded down into context for everything under `components/` to consume.
- **`package.json`** — declares `shadcn-svelte` (or the specific shadcn components used) and the shared shadcn JSONForms renderer package as `peerDependencies` per §14, so a consuming Svelte-native host app supplies its own copies rather than this package bundling redundant copies.

#### `packages/editor-webcomponent/`

- **`src/define-element.ts`** — the only real source file: imports `EditorShell.svelte` from the `editor` package, mounts it inside a `customElements.define('jsonforms-editor', ...)` wrapper class, forwards the `theme`/`locale` attributes into the mounted component's props/context (since Shadow DOM blocks ambient inheritance, §15/§16.3), and — unlike the `editor` package — actually bundles shadcn's components, the shared renderer package, and Monaco's assets into its own build output rather than declaring them as peer dependencies, so the resulting custom element needs zero host-side setup (§14).
- **`package.json`** — no `peerDependencies`; everything the element needs ships in its own bundle.

#### `packages/uischema-meta-schema/`

A minimal package (a JSON file plus a thin `index.ts` re-exporting it, and maybe a generated TypeScript type via `json-schema-to-typescript`) holding the hand-authored JSONForms UI Schema meta-schema from §9/§18.6. Kept standalone specifically so it has exactly one canonical copy, imported both by `MonacoPane.svelte`'s `uischema.json` completion setup and by whichever `inspector/schemas/` file needs to validate/scaffold UI Schema element shapes — avoiding the two consumers drifting out of sync with each other, which was the whole reason §9 called out authoring this file as its own deliverable rather than inlining it wherever first needed.

#### `apps/demo/`

Not part of the publishable packages — a Vite app (or similar) with the mode switch from §14: one code path imports `EditorShell` from `@yourscope/editor` directly (satisfying its peer deps locally, and using shadcn for the demo's own shell chrome too), the other loads `@yourscope/editor-webcomponent` and mounts the `<jsonforms-editor>` tag. Useful as the first place integration bugs between the two packages would actually surface, so it's worth standing this up early (§17's Phase 8) rather than only at the very end.
