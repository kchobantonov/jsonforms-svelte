    ...
    {#each thumbItems as thumb (thumb.index)}
      ...
      {#if showValue}
        <SliderPrimitive.ThumbLabel
          ...
          aria-hidden="true"
          class={cn(
            'pointer-events-none z-50 mb-2 min-w-6 rounded-md border border-border bg-popover px-2 py-1 text-center text-xs font-medium text-popover-foreground shadow-md transition-opacity after:absolute after:top-full after:left-1/2 after:size-2 after:-translate-x-1/2 after:-translate-y-1/2 after:rotate-45 after:border-e after:border-b after:border-inherit after:bg-inherit data-active:visible data-active:opacity-100',
