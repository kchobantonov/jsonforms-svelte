<script lang="ts">
  import { defineShadcnDemoTheme } from '$lib/core/monaco-theme';
  ...
  async function refreshEditorTheme() {
    ...
    const lightThemeName = `shadcn-demo-light-${themeName}`;
    const darkThemeName = `shadcn-demo-dark-${themeName}`;

    // Wait for layout/theme effects to update document attributes and CSS vars.
    await tick();
    await nextFrame();

    monaco.editor.defineTheme(lightThemeName, defineShadcnDemoTheme(false));
    monaco.editor.defineTheme(darkThemeName, defineShadcnDemoTheme(true));
